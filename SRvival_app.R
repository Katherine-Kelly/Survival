        ...
        tabPanel("Summary Statistics",            
                 tabsetPanel(
                     ...
                     tabPanel("COX Proportional Hazard Models", fluidRow(splitLayout(cellWidths = c("33%", "33%", "33%"), plotOutput("ageCox"), plotOutput("subtypeCox"), plotOutput("stageCox")))),
                              tabPanel("Distribution of Data", fluidRow(splitLayout(cellWidths = c("33%", "33%", "33%"),plotOutput("subtypesummary"), plotOutput("agesummary"), plotOutput("gradesummary"))),
                     )
                 )
...
server <- function(input, output) {
    ...
    runSur2 <- reactive({
        req(input$Protein_ID2)
        survfit(as.formula(paste("Surv(time,endpoint) ~ ",paste(input$Protein_ID2))),data=myData())
        
    })
    
    # Plot survival function
    output$plot_survival2 <- renderPlot ({
        plot(runSur2(), col=c('red', 'blue'), xlab='Time (days)', ylab='Probability of Survival', lwd=2, main = c(paste("Kaplan Meier Survival Plot of",paste(input$Protein_ID2))))
        legend(0,0.2, legend=c("High expression", "Low expression"), col=c("red", "blue"),lty=1, lwd=4)
    ...
    diff2 <- reactive ({
        req(input$Protein_ID2)
        diff <- survdiff(as.formula(paste("Surv(time,endpoint) ~" , input$Protein_ID2)), data = myData())
        return(diff)
    })
